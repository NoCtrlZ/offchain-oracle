...
const contract = new ethers.Contract(OracleContractAddress, OracleContractABI, ethereumWallet)
...
const start = async () => {
    ...
    OracleContract.on("RequestCreation", async (url: string, path: string, callbackAddress: string, callbackFunction: string, resType: number, minReporter: number, index: string) => {
        ...
        console.log(await contract.stringResult(index, "hello", [address], []).catch(console.log))
    })
}
