...
const provider = new ethers.providers.JsonRpcProvider(host, { chainId: 1, name: "unknown" })

const DemoContractAddress = contracts.Demo
const DemoContractABI = require("../build/contracts/Demo").abi
const DemoContract = new ethers.Contract(DemoContractAddress, DemoContractABI, provider)

const ethereumWallet = new ethers.Wallet('0x' + privKey, provider)
const contract = new ethers.Contract(DemoContractAddress, DemoContractABI, ethereumWallet)
